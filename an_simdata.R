...
# of simulated datasets (250 per batch) and output the desired estimates
# 
# Input:
# scenario: "basic", "extreme" or "realistic"
# N: batch number (1 - 8)
...
an_simdata <- function(scenario, N, strategy_vec, m, maxit){
  ...
  # Run analysis in 8 batches of 250 each (2000 total)
  for(k in ((N - 1) * 250 + 1):(N * 250)){
    
    # Read in data set
    dat <- read.table(paste("data/simdat_", scenario, "_", paste(sprintf("%04d", k)), ".csv", sep = ""))
    ...
    for(j in 1:n_strat){
      ...
      if(strategy == "quickpred-pt2"){
        ...
        imp <- mice(imp_dat, m = m, maxit = maxit, defaultMethod = c("norm", "logreg", "polyreg", "polr"),
                    pred = quickpred(dat, mincor = 0.2, minpuc = 0, include = "x"),
                    print = FALSE)
      ...
      if(strategy == "quickpred-pt4"){
        ...
        imp_dat <- dat
        imp <- mice(imp_dat, m = m, maxit = maxit, defaultMethod = c("norm", "logreg", "polyreg", "polr"),
                    pred = quickpred(dat, mincor = 0.4, minpuc = 0, include = "x"),
                    print = FALSE)
      ...
      if(strategy == "pcaux"){
        ...
        imp_dat <- select(all_dat, c("y", "x", starts_with("lin")))
      ...
      if(strategy == "forward-fmi"){
        ...
        imp_dat <- cbind(y = dat$y, x = dat$x, aux_dat_selected$A_set)
