/* 
 ...
 Input:
 scenario: string indicating scneario name
 batch: batch number (10 datasets = 1 batch, 200 batches total)
...
program define an_simdata
	...
	* Set up postfile for results 
	...
	postfile results str20 scenario N str20 strategy ///
		...
		using "../../results/analysis/Stata_`scenario'_batch`batch'", replace
		
	local start = (`batch' - 1) * 10 + 1
	local stop = (`batch' * 10)
	...
	forvalues k = `start'(1)`stop' {
		...
		* Read in data set
		...
		di "../../data/`scenario'_`nsim_string'.csv"
		import delimited "../../data/`scenario'_`nsim_string'.csv", ///
		delimiter(space, collapse) varnames(nonames) encoding(ISO-8859-2) clear

		* Name variables correctly
		rename (v1 v2 v3 v4) (index y x z)
		rename v# a#, renumber
		...
		* Change variables to numeric
		destring y x z a1-a`p', ignore("NA") replace
	
		* Save snapshot 
		snapshot save

		* Do MI with forward selection 
		ice y x z a1-a`p', stepwise m(`m') swopts(lock(x z) pe(0.05)) clear
		local vars = r(eq1)
		...
		* Do MI with forward-sw selection
		ice y x z a1-a`p', stepwise m(`m') swopts(lock(x z) forward pe(0.05) pr(0.051)) clear
		local vars = r(eq1)
